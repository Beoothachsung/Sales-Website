...
public class ProcessCart extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        } else if (quantity > cart.get(tid).getProduct().getQuantity()) {
            request.setAttribute("error", cart.get(tid).getProduct().getName()
                    + " only has " + cart.get(tid).getProduct().getQuantity() + " left.");
